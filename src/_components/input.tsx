...
import TagIcon from '@/_components/tags';
...
const Input = ({
    ...
    const onKeydownTag = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            if (!isValidName(value, 1, 10)) {
                alert('태그는 영문(1pt) 또는 한글(2pt)만 사용해 10pt 이내로 작성해주세요.');
                return;
            }
    ...
    return (
        <div className="flex flex-col gap-2.5">
            <label className="text-[var(--black_333236)] text-[var(--font16)] font-regular mb-2" htmlFor="Input">{inputName}</label>
                        ...
                        <>
                            ...
                            <div className="flex absolute top-[110%] left-1 gap-2 mt-2">
                                {Array.from(tags).map((value) => (
                                    <TagIcon key={value} tagName={value} tagStyleType='smallTag' deleteOption={true} onValueChange={getDeleteOrder} />
                                ))}
                            </div>
                        </>
                    ) : (
            ...
            {errorMessage && <div className="text-red-500 mt-1 text-[var(--font14)] font-regular">{errorMessage}</div>}
        </div>
    );
};
